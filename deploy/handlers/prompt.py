...
def build_prompt(prompt: str, labels: str = "",auguments_local=None, auguments_online=None):
    ...
    if labels == "diagnosis":
        ...
        prompts = (
                ...
                f"<|begin_of_text|>\n{Intruction}\n",
                # f"**Hãy trả lời câu hỏi sau**:{prompt.strip()}\n",
                f"Hãy trả lời câu hỏi **{prompt.strip()}** dựa vào nội dung sau:\n* **Chuẩn đoán**:\n{Diagnosis}\n",
                f"* **Triệu chứng**:\n{Symptom}\n",
                f"Lưu ý: Bạn không được trả lời theo nội dung trên mà phải dựa vào kiến thức y khoa của bạn để trả lời đúng nội dung câu hỏi.\n",
                f"<label>{labels.strip()}</label>\n"
        )
    else:
        Intruction = auguments_local["Intruction"]
        ...
        prompts = (
                # f"<|begin_of_text|>\n{Intruction}\nCâu hỏi cần trả lời:{prompt.strip()}\nHãy trình bày câu hỏi dựa vào nội dung sau:\n**Đoạn trích ngắn**:{snippet}\n**Nội dung chính**:{highlight}\nHãy trình bày câu hỏi theo dạng dưới đây nhưng không được lấy theo nội dung:\n{Answer}\n<label>{labels.strip()}</label>\n"
                                f"<|begin_of_text|>\n{Intruction}\n",
                                f"Hãy trả lời câu hỏi **{prompt.strip()}** dựa vào nội dung sau:\n",
                                # f"Hãy trình bày câu hỏi dựa vào nội dung sau:\n",
                                f"* **Câu trả lời**:\n{Answer}\n",
                                f"Lưu ý: Bạn không được trả lời theo nội dung trên mà phải dựa vào kiến thức y khoa của bạn để trả lời đúng nội dung câu hỏi.\n",
                                f"<label>{labels.strip()}</label>\n"
        )
